...
--   "windowrule:RuleName"      → hl.window_rule({name = "RuleName"}):set_enabled(val)
...
local function write_lines(path, lines)
    ...
    if not f then
        print("Error: Unable to write to " .. tmp)
        os.exit(1)
    end
...
local function extract_key(line)
    -- Check for window_rule pattern first
    local wr_name = line:match('^hl%.window_rule%({%s*name%s*=%s*"([^"]+)"%s*}%):set_enabled')
    if wr_name then
        return "windowrule:" .. wr_name
    end
...
local function make_key_pattern(key_parts)
    if key_parts[1] == "windowrule" and key_parts[2] then
        local name = table.concat(key_parts, ":", 2)
        return '^hl%.window_rule%({%s*name%s*=%s*"' .. esc(name) .. '"%s*%}%):set_enabled'
    elseif key_parts[1] == "animation" and key_parts[2] then
...
-- Set a regular hl.config or hl.window_rule option
local function action_set(key, value)
    ...
    local new_line
    if parts[1] == "windowrule" and parts[2] then
        local bool_val = (value == "true" or value == "1") and "true" or "false"
        new_line = string.format('hl.window_rule({name = "%s"}):set_enabled(%s)', parts[2], bool_val)
    else
        new_line = build_config_line(parts, value)
    end
